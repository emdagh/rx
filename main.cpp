...
int main() {
...
#if 1
                next->subscribe([](auto window_value) {
                    DEBUG_VALUE_AND_TYPE_OF(window_value);
                });
#else
                // this causes a SIGSEGV somehow..
                next->to_iterable<std::vector<int>>()->subscribe(
                    [](std::vector<int> window_value) {
                        DEBUG_VALUE_AND_TYPE_OF(window_value);
                    });
#endif
            },
    ...
    rx::range(1, 10)
        ->flat_map<int>([](auto val) {
            return rx::of(val, 3)->delay(10ms)->first()->map([](auto value) {
                return value * value;
            });
        })
        ->subscribe(
